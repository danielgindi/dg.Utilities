...
namespace dg.Utilities.Imaging.Quantizers.Helpers
{
    ...
    public class ImageBuffer : IDisposable
    {
        ...
        public bool ScanForColor(Color ColorToFind, out int FirstX, out int FirstY)
        {
            Pixel pixel = new Pixel(this);
            FirstX = FirstY = -1;
            ...
            if (IsIndexed)
            {
                List<Color> palette = UpdatePalette(false);
                Color[] paletteArray = palette.ToArray();
                if (palette.IndexOf(ColorToFind) < 0) return false;
        ...
        public Color ScanForClosestColor(Color ColorToFind)
        {
            Pixel pixel = new Pixel(this);
            ...
            if (IsIndexed)
            {
                ...
                return minpos == -1 ? Color.Empty : palette[minpos];
            }
            else
            {
                int dr, dg, db, d;
                Color c, foundColor = Color.Empty;
                for (int x = 0, y, w = Width, h = Height; x < w; x++)
                {
                    for (y = 0; y < h; y++)
                    {
        ...
        public static Image QuantizeImage(ImageBuffer source, IColorQuantizer quantizer, IColorDitherer ditherer, Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            ...
            PixelFormat targetPixelFormat = PixelFormatUtility.GetFormatByColorCount(colorCount);
            ...
            {
                source.Quantize(target, quantizer, ditherer, colorCount, (quantizer.AllowParallel && parallelTaskCount > 1) ? parallelTaskCount : 1);
                return result;
            }
        }

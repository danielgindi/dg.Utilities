...
namespace dg.Utilities.Imaging.Encoders
{
    ...
    public class GifEncoder : IDisposable
    {
        ...
        protected System.Drawing.Color TransparentColor
        {
        ...
        protected System.Drawing.Image FrameImage
        {
        ...
        /// </summary>
        /// <param name="FrameImage">Image containing frame to write.</param>
        /// <returns>true if successful.</returns>
        public bool AddFrame(Image FrameImage)
        {
            if ((FrameImage == null) || !_Started)
            {
                return false;
            }
            bool ok = true;
        ...
        protected void AnalyzePixels()
        {
            ...
            using (Image quantizedImage = new Bitmap(_FrameImage.Width, _FrameImage.Height, PixelFormatUtility.GetFormatByColorCount(256)))
            {
                _IndexedPixels = new byte[_FrameImage.Width * _FrameImage.Height];

                using (ImageBuffer quantizedImageBuffer = new ImageBuffer(quantizedImage, ImageLockMode.ReadWrite))
                {
                    using (ImageBuffer sourceImageBuffer = new ImageBuffer(_FrameImage, ImageLockMode.ReadOnly))
                    {
                        hasTransparentColor = _TransparentColor != Color.Empty && sourceImageBuffer.ScanForColor(_TransparentColor, out transparentPixelX, out transparentPixelY);

                        sourceImageBuffer.Quantize(quantizedImageBuffer, quantizer, null, 256, 1);
                        List<Color> palette = quantizedImageBuffer.Palette;
                        ...
                        Pixel pixel = new Pixel(quantizedImageBuffer);
                        j = 0;
                        for (int y = 0, x, w = quantizedImageBuffer.Width, h = quantizedImageBuffer.Height; y < h; y++)
                        {
                            ...
                            {
                                pixel.Update(x, y);
                                if (quantizedImageBuffer.CanRead) quantizedImageBuffer.ReadPixel(pixel);
                        ...
                        if (hasTransparentColor)
                        {
                            ...
                            pixel.Update(transparentPixelX, transparentPixelY);
                            if (quantizedImageBuffer.CanRead) quantizedImageBuffer.ReadPixel(pixel);

...
namespace dg.Utilities.Imaging.Quantizers
{
    /// <summary>
    ...
    public abstract class BaseColorQuantizer : IColorQuantizer
    {
        ...
        protected virtual List<Color> OnGetPalette(Int32 colorCount)
        {
            ...
            if (UniqueColors.Count > 0 && colorCount >= UniqueColors.Count)
            {
                ...
                List<KeyValuePair<int, short>> orderedDictionary = new List<KeyValuePair<int, short>>(UniqueColors);
                orderedDictionary.Sort((pairA, pairB) => pairA.Value.CompareTo(pairB.Value));

                List<Color> colors = new List<Color>(orderedDictionary.Count);
                Color color;
                foreach (KeyValuePair<int, short> pair in orderedDictionary)
                {
                    color = Color.FromArgb(pair.Key);

...
namespace dg.Utilities.Imaging.Quantizers.ColorCaches.LocalitySensitiveHash
{
    /// <summary>
    ...
    public class LshColorCache : BaseColorCache
    {
        ...
        protected override void OnGetColorPaletteIndex(Color color, out Int32 paletteIndex)
        {
            ...
            if (colorCount == 1)
            {
                IEnumerator<int> buckectEnumerator = bucket.Colors.Keys.GetEnumerator();
                buckectEnumerator.MoveNext();
                paletteIndex = buckectEnumerator.Current;
            }
            else
            {
                Int32 index = 0;
                Int32 colorIndex = ColorModelHelper.GetEuclideanDistance(color, ColorModel, new List<Color>(bucket.Colors.Values));
